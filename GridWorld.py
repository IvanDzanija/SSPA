...
class GridWorld:
    ...
    def plot_reward(self):
        unit = 100
        fig, ax = plt.subplots(
            1, 1, figsize=(2 * self.num_cols, 2 * self.num_rows), dpi=300
        )
        ax.axis("off")
                ...
                if self.map[i, j] != 0:
                    ...
                    for a in range(self.num_actions):
                        ...
                        for s_prime in range(len(temp)):
                            if temp[s_prime] > 0:
                                string_action.append(
                                    f"s' = {s_prime}, p = {temp[s_prime]}"
                                )
